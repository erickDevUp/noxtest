...
export default function RelatedArticles({ currentArticle }: { currentArticle: string }) {
    const allPosts = Posts.concat(Featured)
    const RelatedPosts = allPosts.filter(post => post.slug !== currentArticle).slice(0, 3)
    return (
