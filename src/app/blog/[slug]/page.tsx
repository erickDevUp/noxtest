...
export default async function Article({ params }: { params: any }) {
  ...
  return (
    <Stack px={[7,0]} maxWidth={["full",1220]} overflow={["hidden","visible"]} w={"full"} margin={[0,"auto"]} spacing={5} my={[10,16]}>
      <Image src={img} alt="blog-1" width={"full"} height={["fit-content",350]} />
      <Text mt={5} as="span" fontSize={"sm"} fontWeight={"semibold"} color="nox.text-blog-date">{author + " • " + date}</Text>
      <Stack justifyContent={"space-between"} align={"center"} direction={"row"}>
        <Heading as="h1" fontSize={"2xl"} fontWeight={"semibold"} color="nox.text-blog">{title}</Heading>
      </Stack>
...
export function GenericContent() {
  return (
    <Text fontWeight={"medium"} my={5} color="nox.text-blog-light" textAlign={"justify"}>
